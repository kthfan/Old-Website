...
;(function(){
  var MIN_WORKER_ITER = 1, MAX_WORKER_ITER = 1000000;
  ...
  function _setcnd(T, args){
    ...
    else if(T.start==null) T.start = 0;
    ...
    if(args.onfinish!=null) T.onfinish = args.onfinish;
  ...
  function _onfinish(T){
    if(T.isFinish() && T.onfinish) T.onfinish(T);
  }
  /**
    @param  {Integer}  iter
    @param  {Integer}  start
  ...
  */
  self.WebWorkerLoop = function(args){
    args = args || {};
    if (!(this instanceof self.WebWorkerLoop))
      return new self.WebWorkerLoop(args);
    _setcnd(this, args);
    this.workerDataQueue = [];
  }
  WebWorkerLoop.setLocation = function(loc){
    if(loc[loc.length-1]!=="/") loc += "/";
    WebWorkerLoop.iterWorkerPath = loc+"iter_worker.js";
    WebWorkerLoop.integrateWorkerPath = loc + "integrate_worker.js";
  }
  var currentHref = self.location.href;
  WebWorkerLoop.setLocation(currentHref.substr(0,currentHref.lastIndexOf("/")+1));
    ...
    */
    run:function(args){
      ...
      for(var i=0;i<workers_num;i++){
        ...
        for(var j=0;j<T.workerDataQueue.length;j++)
          worker.postMessage([SEND_DATA, T.workerDataQueue[j]]);
      }
      ...
      if(this.integrate_script_path!=null){
        ...
        for(var j=0;i<T.workerDataQueue.length;j++)
          int_worker.postMessage([SEND_DATA, T.workerDataQueue[j]]);
        args.import_list.pop();
        if(this.onmessage){
          ...
          for(let i=0;i<workers_num;i++){
            worker_list[i].onmessage = function(e){
              ...
              if(type === INT_DATA)
                ...
                T._run_obj.worker_finish[i] = true, _onfinish(T);
            }
          }
        }else{
          for(let i=0;i<workers_num;i++){
      ...
      else if(decline_worker)  worker_list[workers_num-1].postMessage([RUN_ITER, worker_start,iter_num+remain_iter]);
